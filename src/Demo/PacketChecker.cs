...
namespace IxyCs.Demo
{
    public class PacketChecker
    {
        ...
        public PacketChecker(string addr1, string addr2)
        {
            InitMempool();
            var dev1 = new IxgbeDevice(addr1, 1, 1);
            var dev2 = new IxgbeDevice(addr2, 1, 1);

            var statsOld = new DeviceStats(dev1);
            var statsNew = new DeviceStats(dev1);
            ulong counter = 0;
            ...
            while(true)
            {
                var buffers = _mempool.AllocatePacketBuffers(1);
                foreach(var buf in buffers)
                    buf.WriteData(PacketSize - 4, seqNum++);
                //dev.TxBatchBusyWait(0, buffers);
                dev1.TxBatch(0, buffers);
                while(true)
                {
                    var received = dev2.RxBatch(0, 1);
                    if(received.Length > 0)
                    {
                        Log.Notice("Received {0} buffers", received.Length);
                        var payload = received[0].CopyData();
                        Log.Notice("Payload size: {0}", payload.Length);
                        return;
                    }
                }
